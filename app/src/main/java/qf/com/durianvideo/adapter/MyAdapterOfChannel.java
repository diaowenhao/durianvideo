    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       ...
       JSONObject jsonObject= channellist.get(position);
        ...
        try {
            ...
            myholder.channelitem_tv_channel.setTag(jsonObject.getString("typeId"));            Glide.with(context).load(jsonObject.getString("typePic").trim()).placeholder(R.mipmap.ic_launcher).transform(new CornersTransform(context,5)).error(android.R.drawable.ic_menu_close_clear_cancel).into(myholder.channelitem_img_channel);
